...
Page({
  ...
  onShow: function () {
    ...
    if (arr.length > 0) {
      for (var i in arr) {
        that.data.total += Number(arr[i].sell_price) * Number(arr[i].count);
  ...
  delCount: function (e) {
    ...
    // 总价钱 减去 对应项的价钱单价
    this.data.total -= Number(this.data.cart[e.target.id.substring(3)].sell_price);
  ...
  delGoods: function (e) {
    ...
    // 总价钱  减去  对应删除项的单价*数量
    this.data.total -= this.data.cart[e.target.id.substring(3)].sell_price * this.data.cart[e.target.id.substring(3)].count;
    // 主体数据的数组移除该项
